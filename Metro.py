...
def update_metro_data(url=metro_data_url):
    ...
    try:
        ...
        # 比较版本号
        version, *_ = load_metro_data(file_path_test)
...
def liststations():
    enabled_stations = []
    disabled_stations = []
    _, stations, _ = load_metro_data(file_path_test)
    for station, info in stations.items():
        if info.get("status") == "enable":
            enabled_stations.append(station)
        else:
            disabled_stations.append(station)
    
    result = f"已启用的地铁站如下：{' '.join(enabled_stations)}"
...
def main():
    ...
    # 解析 metro 可变参数
    if args.metro:
        print(Navigate.navigate_metro_test(*args.metro, file_path))
        return
    if args.liststation:
        print(liststations())
        return
