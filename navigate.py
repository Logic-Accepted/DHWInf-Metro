...
def find_nearest_station(coord, stations):
    ...
    for station, station_coord in stations.items():
        distance = calculate_distance_coords(
            coord, tuple(station_coord['coordinates']))
...
def build_graph(stations, lines):
    ...
    for line, info in lines.items():
        linesinfo[line] = [station for station in info['stations']
                           if station in stationlist]  # 检查 stations 里是否存在这一站
        line_name = info["name"]
    ...
    for line_name, line in linesinfo.items():  # 加入线路名称
        for i in range(len(line) - 1):
            station1 = line[i]
            station2 = line[i + 1]
            distance = calculate_distance_bewteen_station(
...
def navigate_metro(*args):
    ...
    if len(args) == 4:
        ...
        start_station, start_distance = find_nearest_station(
            current_coords, stations)
        end_station, end_distance = find_nearest_station(
            destination_coords, stations)
    ...
    if (start_station != end_station):
        # 计算最短路径
        route, lineList, total_distance = dijkstra(
            graph, start_station, end_station)
        # 格式化输出
        formatted_output = format_route_output(
            route, lineList, start_distance, end_distance, total_distance, lines)
...
def format_route_output(route, lineList, start_distance=0, end_distance=0, total_distance=None, lines=None):
    ...
    for i in range(1, len(route)):
        if lineList[i - 1] == current_line:
            ...
            direction = determine_direction(
                route[i-2], route[i-1], current_line, lines)
            output.append(
    ...
    # 最后一段
    direction = determine_direction(
        route[-1-1], route[-1], lineList[-1], lines)
    output.append(
        f"{first_station}地铁站 \n↓{lines[current_line]['name']['zh']}{direction}方向 乘坐{stationsum}站\n{route[-1]}地铁站\n")
    if end_distance != 0:
    ...
    if total_walk_distance != 0:
        output.append(
            f"总计步行距离约{total_walk_distance:.2f}米，乘车约{total_distance:.0f}米。")
    else:
